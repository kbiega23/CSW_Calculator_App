...
CACHE_BUSTER = "2025-09-17e"

st.set_page_config(page_title="CSW Savings Calculator", page_icon="🪟", layout="centered")


# ================== Navigation helpers (fixes double-click) ==================
def go_next(extra_state: Dict = None):
    if extra_state:
        st.session_state.update(extra_state)
    st.session_state.step = int(st.session_state.get("step", 1)) + 1
    st.rerun()

def go_back():
    st.session_state.step = max(1, int(st.session_state.get("step", 1)) - 1)
    st.rerun()
...
if st.session_state.step == 1:
    ...
    with col1:
        if st.button("Next ➜", key="btn_step1_next"):
            go_next({"location": {"state": state, "city": city, "hdd": hdd, "cdd": cdd}})
    with col2:
        st.button("Reset", on_click=reset_all, key="btn_step1_reset")
...
# ---------- Step 2: Building Type ----------
    ...
    col1, col2 = st.columns(2)
    with col1:
        st.button("◀ Back", on_click=go_back, key="btn_step2_back")
    with col2:
        if st.button("Next ➜", key="btn_step2_next"):
            go_next({"btype": {"building_type": btype_choice, "school_subtype": school_sub}})
...
# ---------- Step 3: Building Details ----------
    ...
    def mf_subtype_from_floors(f: Optional[int]) -> str:
        return "Low-rise Multifamily" if (f is not None and f < 4) else "Mid-rise Multifamily"
    ...
    col1, col2 = st.columns(2)
    with col1:
        st.button("◀ Back", on_click=go_back, key="btn_step3_back")
    with col2:
...
# ---------- Step 4: Rates & Scope ----------
    ...
    col1, col2 = st.columns(2)
    with col1:
        st.button("◀ Back", on_click=go_back, key="btn_step4_back")
    with col2:
...
# ---------- Step 5: Review & Results ----------
    ...
    col1, col2 = st.columns(2)
    with col1:
        st.button("◀ Back", on_click=go_back, key="btn_step5_back")
    with col2:
        st.button("Start Over", on_click=reset_all, key="btn_step5_reset")
